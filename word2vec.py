...
filename=os.path.join(gettempdir(), 'text8.zip')
...
def getdistance(w,x,y,z):	#apply pythagorean theorem to get distance between words
	a=w-y
	b=x-z
	c=(a**2)+(b**2)
...
def getwordcoord(low_dim_embs,labels,word):
	try:
		findword=labels.index(word)
		x, y = low_dim_embs[findword, :]
		return x,y	#get location of word in labels list and find coordinates in low_dim_embs
	except:
		x,y=int(0),int(0)
		return x,y

def balancesentence(s1,s2):		#adds 0's to a sentence vector until both are the same size
	while(len(s1)<len(s2)):
		s1.append(0)
